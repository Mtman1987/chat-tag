    ...
    <div className="rounded-2xl border border-white/10 bg-black/25 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">Scope: #{channel} · {active ? 'ACTIVE' : 'STOPPED'}</p>
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" disabled={busy || active === true} onClick={() => void setRunning(true)} className="rounded-full bg-emerald-300 px-4 py-2 text-xs font-black text-slate-950 disabled:opacity-40">Start</button>
        <button type="button" disabled={busy || active === false} onClick={() => void setRunning(false)} className="rounded-full border border-rose-300/20 bg-rose-300/10 px-4 py-2 text-xs font-bold text-rose-100 disabled:opacity-40">Stop</button>
      </div>
