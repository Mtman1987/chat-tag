...
test('Bingo uses a shared anti-cheat board, transcript triggers, Stella defense, and tiered Games Points purchases', () => {
  ...
  const mic = read('src/components/bingo-transcript-control.tsx');
  const controller = read('src/components/nebula-controller.tsx');
  assert.match(transcript, /isBotRequest/);
  assert.match(transcript, /getSessionUserFromRequest/);
  assert.match(transcript, /signedInChannel !== channel/);
  assert.match(transcript, /Only the broadcaster signed into this channel/);
  assert.match(transcript, /ingestBingoTranscript/);
  assert.match(transcript, /queueStellaSpeech/);
  assert.match(mic, /SpeechRecognition/);
  assert.match(mic, /webkitSpeechRecognition/);
