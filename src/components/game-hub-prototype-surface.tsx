...
const STOP_WORDS = new Set(['this', 'that', 'with', 'have', 'from', 'your', 'just', 'they', 'what', 'when', 'then', 'there', 'here', 'about']);
...
function TeamBoard({ events, paint = false }: { events: GameHubChatEvent[]; paint?: boolean }) {
  const state = useMemo(() => {
    const memberships = new Map<string, string>();
    const scores = Object.fromEntries(TEAM_NAMES.map((team) => [team, 0])) as Record<string, number>;
    for (const event of events) {
      const command = cleanCommand(event.message);
      const team = TEAM_NAMES.find((name) => command === `!${name}`);
      if (team) {
  ...
  return (
    <div className="grid gap-2">
      ...
      <div className="text-[10px] text-white/50">{state.memberships.size} players on teams · use !red !blue !green !yellow</div>
    </div>
  );
}
...
function PixelBoard({ events }: { events: GameHubChatEvent[] }) {
  const cells = useMemo(() => {
    ...
    for (const event of events) {
      const match = cleanCommand(event.message).match(/^(?:paint\s+)?(red|blue|green|yellow|purple|orange|pink|white|black|cyan)\s+(\d{1,2})\s+(\d{1,2})$/);
      if (!match) continue;
...
function TreasureBoard({ events, channel }: { events: GameHubChatEvent[]; channel: string }) {
  ...
  const dug = useMemo(() => {
    ...
    for (const event of events) {
      const match = cleanCommand(event.message).match(/^!dig\s+([a-h])([1-8])$/i);
      if (!match) continue;
...
function WordStorm({ events }: { events: GameHubChatEvent[] }) {
  const words = useMemo(() => {
    ...
    for (const event of events) for (const word of gameWords(event.message)) counts.set(word, (counts.get(word) || 0) + 1);
...
function PhraseGuess({ events, channel }: { events: GameHubChatEvent[]; channel: string }) {
  ...
  const winner = events.find((event) => cleanCommand(event.message) === phrase);
...
function RaceBoard({ events, chicken = false }: { events: GameHubChatEvent[]; chicken?: boolean }) {
  const racers = useMemo(() => {
    ...
    for (const event of events) {
      const command = cleanCommand(event.message);
      const join = command.match(/^!join(?:\s+(dog|cat|rabbit|turtle|hamster))?$/);
      if (join && !map.has(event.username)) map.set(event.username, { name: event.displayName, pet: chicken ? '🐔' : ({ dog: '🐕', cat: '🐈', rabbit: '🐇', turtle: '🐢', hamster: '🐹' } as Record<string, string>)[join[1] || ''] || ['🐕','🐈','🐇','🐢','🐹'][hashText(event.username) % 5], activity: 0, seed: hashText(event.username) % 20 });
      const racer = map.get(event.username); if (racer) racer.activity += 1;
    }
  ...
  return <div className="space-y-2">{racers.length ? racers.map((racer) => <div key={racer.name} className="grid grid-cols-[28px_78px_1fr] items-center gap-2 text-xs"><span>{racer.pet}</span><span className="truncate">{racer.name}</span><span className="h-3 overflow-hidden rounded-full bg-white/10"><i className="block h-full rounded-full bg-cyan-300" style={{ width: `${racer.progress}%` }} /></span></div>) : <div className="text-sm text-white/40">Use !join to enter.</div>}</div>;
}
...
export function GameHubPrototypeSurface({ game, events, channel }: { game: GameHubGame; events: GameHubChatEvent[]; channel: string }) {
  ...
  const content = useMemo(() => {
    if (game.id === 'chaosmode') {
      const special = recent.filter((event) => /^!(explode|glitch|portal|shake)$/i.test(cleanCommand(event.message))).at(-1);
      const level = Math.min(100, recent.length * 4);
      return <div className="grid min-h-36 place-items-center text-center"><div><div className="text-6xl">{special ? '💥' : level > 70 ? '🌀' : '⚡'}</div><div className="mt-2 text-2xl font-black">CHAOS {level}%</div><div className="mt-1 text-[10px] text-white/50">{special ? `${special.displayName}: ${special.message}` : 'Every message raises the chaos.'}</div></div></div>;
    }
    if (game.id === 'chatgarden') {
      const plants = recent.flatMap((event) => PLANTS.filter(([pattern]) => pattern.test(event.message)).map(([, icon]) => ({ icon, user: event.displayName }))).slice(-24);
    ...
    if (game.id === 'chatwars') return <TeamBoard events={recent} />;
    if (game.id === 'colorwars') return <TeamBoard events={recent} paint />;
    ...
    if (game.id === 'emojirain') {
      const emojis = recent.flatMap((event) => emojiTokens(event.message).map((emoji) => ({ emoji, id: `${event.id}-${emoji}` }))).slice(-40);
    ...
    if (game.id === 'emojitower') {
      const drops = recent.filter((event) => cleanCommand(event.message) === '!drop').slice(-18);
      return <div className="flex min-h-40 flex-col-reverse items-center justify-start gap-0.5">{drops.length ? drops.map((event, index) => <span key={event.id} className="grid h-7 place-items-center rounded border border-white/15 bg-violet-400/15 text-xl" style={{ width: `${50 + (hashText(event.id) % 70)}px`, transform: `translateX(${(hashText(`${event.id}:x`) % 31) - 15}px)` }}>{['🟪','🟦','🟩','🟨','🟥'][index % 5]}</span>) : <span className="my-auto text-sm text-white/40">!drop stacks the next block.</span>}</div>;
    }
    if (game.id === 'dancingparade') {
      const dancers = new Map<string, GameHubChatEvent>();
      for (const event of recent) { const command = cleanCommand(event.message); if (command === '!join' || command === '!dance') dancers.set(event.username, event); if (command === '!leave') dancers.delete(event.username); }
      return <div className="flex min-h-36 flex-wrap items-end justify-center gap-4">{dancers.size ? [...dancers.values()].map((event) => <div key={event.username} className="text-center"><div className="animate-bounce text-4xl">🕺</div><div className="text-[10px]">{event.displayName}</div></div>) : <span className="self-center text-sm text-white/40">!join the parade.</span>}</div>;
    }
    if (game.id === 'memorylane') {
      const memories = recent.filter((event) => event.message.length >= 24).slice(-5);
    ...
    if (game.id === 'colorsymphony') {
      const notes = recent.flatMap((event) => Object.keys(COLORS).filter((color) => new RegExp(`\\b${color}\\b`, 'i').test(event.message)).map((color) => ({ color, id: `${event.id}-${color}` }))).slice(-16);
    ...
    if (game.id === 'rhythmpulse') {
      const bars = recent.slice(-20).map((event) => Math.min(100, 12 + event.message.length * 2 + emojiTokens(event.message).length * 12));
      return <div className="flex min-h-36 items-end justify-center gap-1">{bars.length ? bars.map((value, index) => <span key={`${recent[recent.length - bars.length + index]?.id}-${index}`} className="w-3 rounded-t bg-cyan-300/80" style={{ height: `${value}%`, minHeight: '8px' }} />) : <span className="self-center text-sm text-white/40">Chat creates the beat.</span>}</div>;
    }
    return <div className="grid min-h-36 place-items-center text-sm text-white/45">Live chat runtime connected.</div>;
  }, [channel, game.id, recent]);

  return <section className="h-full min-h-0 overflow-hidden rounded-2xl border border-white/15 bg-slate-950/70 p-4 text-white shadow-2xl backdrop-blur"><header className="mb-3 flex items-center justify-between gap-2"><div><div className="text-[9px] uppercase tracking-[.18em] text-cyan-200/60">Games Hub</div><h2 className="font-bold">{game.name}</h2></div><span className="rounded-full bg-white/8 px-2 py-1 text-[9px] text-white/55">#{channel}</span></header>{content}</section>;
}
