...
const MANAGEMENT_LINKS = [
  ...
  { label: 'Overlay', href: `${SPMT_ORIGIN}/embed/overlays?mode=full&app=chat-tag&output=personal`, Icon: PanelsTopLeft },
...
export function SpmtWorkspaceHost() {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setConnected(false); setTokens(null); setTenantOutputs(null); setPersonalOverlayUrl(''); setLoaded(true); return;
      }
      ...
      setActiveSlotId((current) => nextTokens.dockSlots?.some((slot) => slot.id === current) ? current : (slots[0]?.id ?? nextTokens.dockSlots?.[0]?.id ?? null));
      setConnected(true); setLoaded(true);
    } catch {
      setConnected(false); setTokens(null); setTenantOutputs(null); setPersonalOverlayUrl(''); setLoaded(true);
    }
  }, [hiddenRoute]);
  ...
  const openSlot = (slot: WorkspaceDockSlotV1) => { if (!connected) { setOpen(true); return; } setActiveSlotId(slot.id); setOpen(true); };
  const togglePersonalOverlay = () => setPersonalOverlayVisible((current) => {
    const next = !current;
    ...
    {footerVisible && open && <section className="fixed inset-x-3 bottom-[76px] z-[100] mx-auto flex h-[min(72vh,700px)] max-w-5xl flex-col overflow-hidden rounded-2xl border border-white/15 bg-black/90 shadow-2xl backdrop-blur-xl sm:inset-x-6">
      <header className="flex flex-wrap items-center gap-2 border-b border-white/10 p-2">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1">
          {connected ? traySlots.map((slot) => <button key={slot.id} type="button" onClick={() => setActiveSlotId(slot.id)} className={`rounded-lg px-3 py-2 text-sm font-medium ${activeSlot?.id === slot.id ? 'bg-white/15 text-white' : 'text-white/65 hover:bg-white/10 hover:text-white'}`}>{slot.title || `Slot ${slot.id}`}</button>) : <span className="px-2 text-xs font-semibold text-amber-200">SPMT workspace disconnected</span>}
        </div>
      ...
      {!loaded ? <div className="flex flex-1 flex-col items-center justify-center gap-3 p-8 text-center text-white/70"><RefreshCw className="h-7 w-7 animate-spin" aria-hidden /><p className="text-sm">Connecting to your SPMT workspace…</p></div>
        : !connected ? <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center text-white/75"><LayoutGrid className="h-9 w-9 text-amber-300" aria-hidden /><div><p className="font-semibold text-white">ChatTag is open, but the shared SPMT workspace is not connected.</p><p className="mt-1 max-w-lg text-sm text-white/55">Reconnect SPMT to restore your Worktray, workspace URLs, shared appearance, and Personal overlay.</p></div><a href={reconnectHref} className="rounded-lg bg-cyan-300 px-4 py-2 text-sm font-bold text-slate-950 hover:bg-cyan-200">Reconnect SPMT workspace</a></div>
    ...
    {footerVisible ? <aside className="fixed inset-x-3 bottom-3 z-[110] mx-auto max-w-5xl overflow-hidden rounded-2xl border border-white/15 bg-black/80 shadow-[0_-14px_42px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:inset-x-6" aria-label="SPMT workspace tray" data-workspace-footer="true">
      <div className="flex min-h-14 items-center gap-2 p-2">
        ...
        <div className="grid min-w-0 flex-1 grid-cols-3 gap-1.5">
          {traySlots.map((slot) => <button key={slot.id} type="button" onClick={() => openSlot(slot)} className={`min-w-0 rounded-xl border px-2.5 py-2 text-left ${connected && activeSlot?.id === slot.id && open ? 'border-cyan-300/35 bg-cyan-300/[0.08]' : 'border-white/10 bg-white/[0.025] hover:bg-white/[0.06]'}`}><span className="block truncate text-[10px] font-bold text-white">{connected ? (slot.title || `Slot ${slot.id}`) : `Slot ${slot.id}`}</span><span className="mt-0.5 block text-[8px] font-semibold uppercase tracking-wide text-white/35">{connected ? (slot.collapsed ? 'Hidden' : `Slot ${slot.id}`) : 'SPMT offline'}</span></button>)}
        </div>
